...
class Products extends CMS{
	...
	public function showProducts($id = null, $category = null, $userID = null){
		...
		if(isset($userID))					
			$result = $conn->query("SELECT * FROM $dbTable WHERE user_id = '$userID' ORDER BY id DESC LIMIT $offset, $rowsPerPage");
	...
	public function showProductsManager(){ 
		...
		$userID = $userID_row[0];
		$user_group = $userID_row[1];
		
		if($user_group >= 3)					
			$output .= $display->listTable($tablename, $rowsPerPage, "admin");
		else
			...
			if(isset($submit)){
				...
				if(!empty($v_name)){
					...
					if($_FILES['picture']['size'] > 0){
						...
						$result = $conn->query("INSERT INTO ".$this->getDBTable($tablename)." (id, user_id, name, category, picture, size, color, price, quantity, description) VALUES (NULL, '$userID','$v_name', '$v_category','$upload', '$v_size', '$v_color', '$v_price', '$v_quantity', '$v_description')");
						...
						$result = $conn->query("INSERT INTO ".$this->getDBTable($tablename)." (id, user_id, name, category, picture, size, color, price, quantity, description) VALUES (NULL, '$userID', '$v_name', '$v_category','$v_picture', '$v_size', '$v_color', '$v_price', '$v_quantity', '$v_description')");
